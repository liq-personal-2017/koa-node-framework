...
exports.getClazzMapping = undefined;
...
let Mapping = class Mapping {
     ...
     */
    constructor(mapping) {
        (0, _assert2.default)(validMapping(mapping), 'request-mapping value must be string or object {url:string,method(s):"post"/"get"/"put"/"delete"/"patch"}');
        if ((0, _lodash.isString)(mapping)) {
            this.url = mapping;
            this.method = ['all'];
        } else {
...
function mapClass(value, target) {
    (0, _assert2.default)((0, _lodash.isString)(value), 'class mapping must be string');
...
function validMethod(value) {
    return Reflect.has(value, 'method') && (0, _lodash.isString)(Reflect.get(value, 'method')) && Reflect.get(value, 'method').split(',').every(method => METHODS.includes(method));
}
/**
 * 映射关系注解
 *  对于类注解
...
exports.getClazzMapping = getClazzMapping;
