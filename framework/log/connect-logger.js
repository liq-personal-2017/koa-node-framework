    ...
    return async (ctx, next) => {
        ...
        function done(event, ctx) {
            ...
            if (ctx.status && options.level === 'auto') {
                level = levels.INFO
                if (ctx.status >= 300) level = levels.WARN
                if (ctx.status >= 400) level = levels.ERROR
            }
...
function assembleTokens(ctx, customTokens) {
    ...
    defaultTokens.push({
        token: ':remote-addr',
        replacement: getRemoteAddressFromContext(ctx),
    })
