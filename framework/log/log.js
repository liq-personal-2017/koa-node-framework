...
import { times, last, toPairs, isString } from 'lodash'
...
const logRootPath = path.join(process.cwd(), 'log')
...
class FileAppender {
    ...
    constructor(name, appender) {
        ...
        this.compress = !!decode(appender.compress, undefined, DEFAULT_COMPRESS_LOG)
    ...
    getFullFilename() {
        ...
        this.filename = path.join(logRootPath, filename)
    }
    /**
     * 处理日志文件的大小
     *
...
class Log4jsConfig {
    ...
    makeAppenders(apps) {
        if (apps) {
            ...
            for (let [ name, app, ] of toPairs(apps)) {
    ...
    makeCategories(cats) {
        if (cats) {
            ...
            for (let [ name, cat, ] of toPairs(cats)) {
                // 不允许修改默认的日志记录器
                if (name == 'default') {
...
let logger = new Logger(log4js.getLogger())
...
export { Logger, createLoggers, createLogger }
/**
    let config = {
        appenders: {
            common: {
